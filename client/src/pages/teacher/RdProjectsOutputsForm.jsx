...
import { createRdProjectsOutput } from "../../../config/api";
import Alert from "../../components/Alert";
import SearchableSelect from "../../components/SearchableSelect";
import {
  RD_PROJECTS_OUTPUTS_STORAGE_KEY,
  rdProjectsOutputsAttachmentConditionalFields,
  rdProjectsOutputsDisplayStructure,
  rdProjectsOutputsFields,
  rdProjectsOutputsMaxWordsByKey,
...
function RdProjectsOutputsForm() {
  const fields = useMemo(() => rdProjectsOutputsFields, []);
  ...
  const [formValues, setFormValues] = useState(() => {
    ...
    try {
      const rawStoredValues = window.localStorage.getItem(RD_PROJECTS_OUTPUTS_STORAGE_KEY);
      if (!rawStoredValues) {
        return initialValues;
      }
  ...
  const stepSections = useMemo(
    () =>
      rdProjectsOutputsDisplayStructure.map((group) => ({
        ...group,
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(RD_PROJECTS_OUTPUTS_STORAGE_KEY, JSON.stringify(formValues));
  }, [formValues]);
  ...
  const handleChange = (field, value) => {
    if (rdProjectsOutputsMaxWordsByKey[field.key]) {
      const limit = rdProjectsOutputsMaxWordsByKey[field.key];
  ...
  const validate = () => {
    ...
    Object.entries(rdProjectsOutputsAttachmentConditionalFields).forEach(
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await createRdProjectsOutput(formData, token);
      window.localStorage.removeItem(RD_PROJECTS_OUTPUTS_STORAGE_KEY);
      setFormValues(initialValues);
      setErrors({});
      setCurrentStepIndex(0);
      showAlert("R&D Projects & Outputs entry submitted successfully.", "success");
    } catch (error) {
      showAlert(error.message || "Failed to submit R&D Projects & Outputs entry.", "error");
    } finally {
    ...
    if (rdProjectsOutputsMaxWordsByKey[field.key]) {
      return (
        <p className="text-xs text-gray-500">
          Max: {rdProjectsOutputsMaxWordsByKey[field.key]} words
        </p>
      );
    }
  ...
  const shouldShowField = (fieldKey) => {
    const dependencyKey = rdProjectsOutputsAttachmentConditionalFields[fieldKey];
    if (!dependencyKey) {
      return true;
    }
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">
              Submit R & D Projects & Outputs
            </h1>
            <p className="mt-1 max-w-2xl text-sm leading-6 text-[#7a80a6]">
              Submit your R & D projects and outputs details for faculty review and repository tracking.
            </p>
          </div>
...
export default RdProjectsOutputsForm;
