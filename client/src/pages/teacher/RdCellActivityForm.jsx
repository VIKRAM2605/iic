...
import { createRdCellActivity } from "../../../config/api";
import Alert from "../../components/Alert";
import {
  RD_CELL_ACTIVITY_STORAGE_KEY,
  rdCellActivityAttachmentConditionalFields,
  rdCellActivityDisplayStructure,
  rdCellActivityFields,
  rdCellActivityMaxWordsByKey,
...
function RdCellActivityForm() {
  const fields = useMemo(() => rdCellActivityFields, []);
  ...
  const stepSections = useMemo(
    () =>
      rdCellActivityDisplayStructure.map((group) => ({
        ...group,
        fields: group.fields
  ...
  const handleChange = (field, value) => {
    if (rdCellActivityMaxWordsByKey[field.key]) {
      const limit = rdCellActivityMaxWordsByKey[field.key];
  ...
  const validate = () => {
    ...
    Object.entries(rdCellActivityAttachmentConditionalFields).forEach(
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await createRdCellActivity(formData, token);
      window.localStorage.removeItem(RD_CELL_ACTIVITY_STORAGE_KEY);
      setFormValues(initialValues);
      setErrors({});
      setCurrentStepIndex(0);
      showAlert("R&D Cell activity submitted successfully.", "success");
    } catch (error) {
      showAlert(error.message || "Failed to submit R&D Cell activity.", "error");
    } finally {
    ...
    if (rdCellActivityMaxWordsByKey[field.key]) {
      return (
        <p className="text-xs text-gray-500">
          Max: {rdCellActivityMaxWordsByKey[field.key]} words
        </p>
      );
    }
      ...
      {field.type === "textarea" && (
        <div className="space-y-1">
          <textarea
            ...
            rows={4}
          />
          {rdCellActivityMaxWordsByKey[field.key] && (
            <p
              className={`text-xs ${
                countWords(formValues[field.key]) / rdCellActivityMaxWordsByKey[field.key] >= 0.8
                  ? "text-red-600"
                  : "text-gray-500"
              }`}
            >
              {countWords(formValues[field.key])}/{rdCellActivityMaxWordsByKey[field.key]}
            </p>
          )}
        </div>
      )}
  ...
  const shouldShowField = (fieldKey) => {
    const dependencyKey = rdCellActivityAttachmentConditionalFields[fieldKey];
    if (!dependencyKey) {
      return true;
    }
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">
              Submit R & D Cell Activity
            </h1>
            <p className="mt-1 max-w-2xl text-sm leading-6 text-[#7a80a6]">
              Submit your R&amp;D Cell activity with sub theme, title, and level
              duration details.
            </p>
          </div>
...
export default RdCellActivityForm;
