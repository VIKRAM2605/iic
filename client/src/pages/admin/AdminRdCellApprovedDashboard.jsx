...
import {
  getAdminApprovedRdCellNominationFilterOptions,
  getAdminApprovedRdCellNominations,
...
export default function AdminRdCellApprovedDashboard() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      try {
        const [nominationsPayload, optionsPayload] = await Promise.all([
          getAdminApprovedRdCellNominations({ token, includeRejected: true }),
          getAdminApprovedRdCellNominationFilterOptions(token, {
            includeRejected: true,
          }),
        ]);

        setNominations(nominationsPayload.data || []);
        setOptions({
        ...
        setAlertState({
          isOpen: true,
          message: error.message || "Failed to fetch R&D nominations.",
          severity: "error",
        });
      } finally {
        setLoading(false);
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted">
              Review approved and rejected R & D Cell nominations
            </p>
          </div>
