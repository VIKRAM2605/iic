...
import {
  rdFacilitiesServicesDetailSteps,
  rdFacilitiesServicesFieldLabels,
  ...
  Object.entries(details).map(([key, value]) => (
    <div key={key} className="rounded-md border border-gray-200 p-3">
      <p className="text-xs font-semibold uppercase text-gray-500">
        {rdFacilitiesServicesFieldLabels[key] || prettifyKey(key)}
      </p>
      <p className="mt-1 wrap-break-word text-sm text-gray-800">{renderValue(value)}</p>
    </div>
  ));

export default function RdFacilitiesServiceDetails() {
  ...
  const backTo =
    typeof location.state?.from === "string" && location.state.from.startsWith("/")
      ? location.state.from
      : isAdmin
        ? "/admin/rd-facilities-services-review"
        : "/teacher/rd-facilities-services";
  ...
  const detailStepProgress =
    rdFacilitiesServicesDetailSteps.length > 1
      ? (activeDetailStep / (rdFacilitiesServicesDetailSteps.length - 1)) * 100
      : 0;
  ...
  useEffect(() => {
    const loadFacility = async () => {
      ...
      setLoading(true);
      try {
        const payload = await getRdFacilitiesServiceById({ token, facilityId });
        setFacilityData(payload.data || null);
  ...
  const handleReview = async (action) => {
    ...
    try {
      const payload = await reviewRdFacilitiesServiceByAdmin({
        token,
        facilityId,
        action,
        ...
        {facilityData?.status && (
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${
              statusBadgeClass[facilityData.status] || "bg-gray-100 text-gray-700"
            }`}
          >
            {facilityData.status}
          </span>
        ...
        <div className="space-y-6">
          <div className="rounded-md border border-gray-200 p-5">
            <h2 className="text-lg font-semibold text-gray-900">
              {facilityData.eventName || `Entry #${facilityData.id}`}
            </h2>
            <div className="mt-4 grid gap-3 text-xs text-gray-700 md:grid-cols-2 xl:grid-cols-4">
              <p><span className="font-semibold">Facility name:</span> {facilityData.eventName || "-"}</p>
              <p><span className="font-semibold">Department:</span> {facilityData.facilityDetails?.serviceRole || "-"}</p>
              <p><span className="font-semibold">Research R&D Focus area:</span> {facilityData.facilityDetails?.serviceFocusArea || "-"}</p>
              <p><span className="font-semibold">Equipments:</span> {facilityData.facilityDetails?.equipments || "-"}</p>
              <p><span className="font-semibold">Name:</span> {facilityData.facilityDetails?.contactName || "-"}</p>
              <p><span className="font-semibold">Email:</span> {facilityData.facilityDetails?.contactEmail || "-"}</p>
              <p><span className="font-semibold">Phone:</span> {facilityData.facilityDetails?.phoneNumber || "-"}</p>
              <p><span className="font-semibold">Owner:</span> {facilityData.ownerName || "-"}</p>
              <p><span className="font-semibold">Owner Email:</span> {facilityData.ownerEmail || "-"}</p>
              <p><span className="font-semibold">Submitted:</span> {facilityData.createdAt ? new Date(facilityData.createdAt).toLocaleDateString() : "-"}</p>
              <p><span className="font-semibold">Rejection Msg:</span> {facilityData.rejectionMessage || "-"}</p>
            </div>
          </div>
          ...
          <div className="rounded-md border border-gray-200 p-5">
            ...
            <div className="mb-4 overflow-x-auto">
              <div className="relative min-w-190 px-2 pb-1">
                ...
                <div className="relative flex items-start justify-between gap-2">
                  {rdFacilitiesServicesDetailSteps.map((step, index) => {
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {renderDetails(facilityData[rdFacilitiesServicesDetailSteps[activeDetailStep]?.key] || {})}
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">
                Step {activeDetailStep + 1} of {rdFacilitiesServicesDetailSteps.length}
              </span>
              <button
                type="button"
                onClick={() =>
                  setActiveDetailStep((previous) =>
                    Math.min(rdFacilitiesServicesDetailSteps.length - 1, previous + 1),
                  )
                }
                disabled={activeDetailStep === rdFacilitiesServicesDetailSteps.length - 1}
