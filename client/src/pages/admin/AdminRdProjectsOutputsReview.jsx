...
import { getAdminRdProjectsOutputsReviewQueue } from "../../../config/api";
import Alert from "../../components/Alert";
import SearchableSelect from "../../components/SearchableSelect";
import { getAuthToken } from "../../utils/auth";

export default function AdminRdProjectsOutputsReview() {
  ...
  useEffect(() => {
    const loadQueue = async () => {
      setLoading(true);
      try {
        const payload = await getAdminRdProjectsOutputsReviewQueue(token);
  ...
  const facultyOptions = useMemo(() => {
    ...
    projects.forEach((item) => {
      [item.ownerName, item.ownerEmail, item.eventName, item.principalInvestigation, item.department, item.trlLevel, item.ipStatus, item.projectStatus]
        .filter(Boolean)
  ...
  const filteredProjects = useMemo(
    () =>
      projects.filter((item) => {
        ...
        return searchableFields.some((value) => value.includes(facultyName.toLowerCase()));
      }),
    [projects, facultyName],
  );
    ...
    <section className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="border-b border-gray-200 bg-white px-8 py-8">
        <div className="space-y-2">
          <h1 className="heading-xl">R & D Projects & Outputs Review Queue</h1>
          <p className="text-muted">Review pending and rejected R & D Projects & Outputs entries</p>
        </div>
      </div>
      ...
      <div className="border-b border-gray-200 bg-white px-8 py-6">
        ...
        <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div />
          <button type="button" onClick={() => setFacultyName("")} className="btn-reset-custom" disabled={loading}>
            Reset
          </button>
        </div>
      </div>
      ...
      <div className="px-8 py-8">
        ...
        {!loading && filteredProjects.length === 0 && (
          <div className="empty-state">
            ...
            <p className="empty-state-description">All pending entries have been reviewed.</p>
          </div>
        )}
