...
import { getFacultyMyPrototypes, getPrototypeById } from "../../../config/api";
...
export default function TeacherPrototypesDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const payload = await getFacultyMyPrototypes(token);
        setEvents(payload.data || []);
      } catch (error) {
        setAlertState({
          isOpen: true,
          message: error.message || "Failed to fetch your prototypes.",
          severity: "error",
        });
      } finally {
        setLoading(false);
  ...
  const handleDownloadReport = async (eventItem) => {
    ...
    try {
      ...
      setAlertState({
        isOpen: true,
        message: "Prototype report downloaded successfully.",
        severity: "success",
      });
    } catch (error) {
      setAlertState({
