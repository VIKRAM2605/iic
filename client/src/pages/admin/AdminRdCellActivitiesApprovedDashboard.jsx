...
import {
  getAdminApprovedRdCellActivities,
  getAdminApprovedRdCellActivitiesFilterOptions,
...
export default function AdminRdCellActivitiesApprovedDashboard() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      try {
        const [activitiesPayload, optionsPayload] = await Promise.all([
          getAdminApprovedRdCellActivities({ token, includeRejected: true }),
          getAdminApprovedRdCellActivitiesFilterOptions(token, {
        ...
        setAlertState({
          isOpen: true,
          message: error.message || "Failed to fetch R&D activities.",
          severity: "error",
        });
      } finally {
        setLoading(false);
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted">
              Review approved and rejected R & D Cell activities
            </p>
          </div>
