...
import {
  rdCellActivityDetailSteps,
  rdCellActivityFieldLabels,
} from "../../constants/rdCellActivity";
  ...
  Object.entries(details).map(([key, value]) => (
    <div key={key} className="rounded-md border border-gray-200 p-3">
      <p className="text-xs font-semibold uppercase text-gray-500">
        {rdCellActivityFieldLabels[key] || prettifyKey(key)}
      </p>
...
export default function RdCellActivityDetails() {
  ...
  const backTo =
    typeof location.state?.from === "string" && location.state.from.startsWith("/")
      ? location.state.from
      : isAdmin
        ? "/admin/rd-cell-activities-review"
        : "/teacher/rd-cell-activities";
  ...
  const detailStepProgress =
    rdCellActivityDetailSteps.length > 1
      ? (activeDetailStep / (rdCellActivityDetailSteps.length - 1)) * 100
      : 0;
  ...
  useEffect(() => {
    const loadActivity = async () => {
      ...
      setLoading(true);
      try {
        const payload = await getRdCellActivityById({ token, activityId });
        setActivityData(payload.data || null);
  ...
  const handleReview = async (action) => {
    ...
    try {
      const payload = await reviewRdCellActivityByAdmin({
        token,
        activityId,
        action,
        ...
        {activityData?.status && (
          <span
            className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${
              statusBadgeClass[activityData.status] || "bg-gray-100 text-gray-700"
            }`}
          >
            {activityData.status}
          </span>
        ...
        <div className="space-y-6">
          <div className="rounded-md border border-gray-200 p-5">
            <h2 className="text-lg font-semibold text-gray-900">
              {activityData.eventName || `Activity #${activityData.id}`}
            </h2>
            <div className="mt-4 grid gap-3 text-xs text-gray-700 md:grid-cols-2 xl:grid-cols-4">
              <p><span className="font-semibold">Sub Theme:</span> {activityData.eventName || "-"}</p>
              <p><span className="font-semibold">Title:</span> {activityData.activityDetails?.title || "-"}</p>
              <p><span className="font-semibold">Level Duration:</span> {activityData.activityDetails?.levelDuration || "-"}</p>
              <p><span className="font-semibold">Owner:</span> {activityData.ownerName || "-"}</p>
              <p><span className="font-semibold">Owner Email:</span> {activityData.ownerEmail || "-"}</p>
              <p><span className="font-semibold">Submitted:</span> {activityData.createdAt ? new Date(activityData.createdAt).toLocaleDateString() : "-"}</p>
              <p><span className="font-semibold">Rejection Msg:</span> {activityData.rejectionMessage || "-"}</p>
            </div>
          </div>
          ...
          <div className="rounded-md border border-gray-200 p-5">
            ...
            <div className="mb-4 overflow-x-auto">
              <div className="relative min-w-190 px-2 pb-1">
                ...
                <div className="relative flex items-start justify-between gap-2">
                  {rdCellActivityDetailSteps.map((step, index) => {
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {renderDetails(activityData[rdCellActivityDetailSteps[activeDetailStep]?.key] || {})}
            </div>
            ...
            <div className="mt-4 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">
                Step {activeDetailStep + 1} of {rdCellActivityDetailSteps.length}
              </span>
              <button
                type="button"
                onClick={() =>
                  setActiveDetailStep((previous) =>
                    Math.min(rdCellActivityDetailSteps.length - 1, previous + 1),
                  )
                }
                disabled={activeDetailStep === rdCellActivityDetailSteps.length - 1}
