...
import { getAdminRdEquipmentsServicesReviewQueue } from "../../../config/api";
import Alert from "../../components/Alert";
import SearchableSelect from "../../components/SearchableSelect";
import { getAuthToken } from "../../utils/auth";

export default function AdminRdEquipmentsServicesReview() {
  ...
  useEffect(() => {
    const loadQueue = async () => {
      setLoading(true);
      try {
        const payload = await getAdminRdEquipmentsServicesReviewQueue(token);
  ...
  const facultyOptions = useMemo(() => {
    ...
    services.forEach((item) => {
      [item.ownerName, item.ownerEmail, item.eventName, item.department, item.utilizationRate, item.suitableDays]
        .filter(Boolean)
    ...
    <section className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="border-b border-gray-200 bg-white px-8 py-8">
        <div className="space-y-2">
          <h1 className="heading-xl">R & D Equipments Services Review Queue</h1>
          <p className="text-muted">Review pending and rejected R & D Equipments Services entries</p>
        </div>
      </div>
