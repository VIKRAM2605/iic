...
import { getFacultyMyIicApplied, getIicAppliedById } from "../../../config/api";
...
export default function TeacherIicAppliedDashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const payload = await getFacultyMyIicApplied(token);
        setEntries(payload.data || []);
      } catch (error) {
        setAlertState({
          isOpen: true,
          message: error.message || "Failed to fetch your IIC Applied entries.",
          severity: "error",
        });
      } finally {
        setLoading(false);
  ...
  const statusOptions = useMemo(
    () => Array.from(new Set(entries.map((item) => item.status).filter(Boolean))),
    [entries],
  );
      ...
      <div className="border-b border-gray-200 bg-white px-8 py-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted">View and manage your IIC Applied submissions</p>
          </div>
