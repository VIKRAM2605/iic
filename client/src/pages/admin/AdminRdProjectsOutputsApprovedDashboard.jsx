...
import {
  getAdminApprovedRdProjectsOutputs,
  getAdminApprovedRdProjectsOutputsFilterOptions,
...
export default function AdminRdProjectsOutputsApprovedDashboard() {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      setLoading(true);
      try {
        const [projectsPayload, optionsPayload] = await Promise.all([
          getAdminApprovedRdProjectsOutputs({ token, includeRejected: true }),
          getAdminApprovedRdProjectsOutputsFilterOptions(token, { includeRejected: true }),
        ]);
      ...
      <div className="border-b border-gray-200 bg-white px-8 py-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted">Review approved and rejected R & D Projects & Outputs entries</p>
          </div>
          <Link
                    ...
                    <td className="px-4 py-3.5 text-center">
                      <span
                        className={`inline-block rounded-full px-3 py-1 text-xs font-semibold capitalize ${
                          item.status === "rejected" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"
                        }`}
                      >
